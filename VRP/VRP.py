...
with open("./solomon_25/toy.txt", 'r') as f:
    lines = f.readlines()
...
veh = list(range(2))              # list of vehicles
...
# costs ci,j
c = [[((xycli[i][0]-xycli[j][0])**2 + (xycli[i][1]-xycli[j][1])**2)**0.5 for i in nodes[:-1]] for j in nodes[:-1]]
c.append(c[0])
...
obj = quicksum(x[i,j,k]*c[i][j] for i in cli for j in cli for k in veh )
# Objetctive Function
m.setObjective(
    quicksum(x[i,j,k]*c[i][j] for i in cli for j in cli for k in veh if i!=j),
    # obj,
    GRB.MINIMIZE
)
